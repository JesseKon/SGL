...
auto main(int argc, char** argv) -> int try {
    ...
    SGL::Window window({ 640, 480 }, u8"Otsikko");
    SGL::Camera camera(window, SGL::CameraType::Orthographic);

    // Load default line shader
    SGL::ShaderGLSL defaultLineShader(
        "../assets/shaders/default/DefaultLineShader.vert",
        "../assets/shaders/default/DefaultLineShader.frag"
    );
    ...
    while (window.getRenderer()->running()) {
        ...
        if (window.getInput()->getKey(SGL::Keycode::Key_A))
            line.translate(SGL::Vector3<float>::left() * 0.5f);

        if (window.getInput()->getKeyPressed(SGL::Keycode::Key_B))
            line.setLocalScaleAround({ 200.0f, 200.0f, 1.0f }, { 2.0f, 2.0f, 1.0f });

        if (window.getInput()->getKeyPressed(SGL::Keycode::Key_C))
            line.setLocalScaleAround({ 200.0f, 200.0f, 1.0f }, { 1.0f, 1.0f, 1.0f });

        window.getRenderer()->beginDrawing(true, true, true, SGL::COLOR::White);
        line.draw();
